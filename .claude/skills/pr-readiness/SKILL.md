...
- [ ] No concrete types (classes, records, structs) defined in `Balenthiran.LanguageVocab.Abstractions`
- [ ] Service interface methods return **interfaces**, not concrete types (`IRatio`, not `Ratio`)
- [ ] `Balenthiran.LanguageVocab.Abstractions.csproj` has **zero** `<ProjectReference>` entries to projects with concrete types
...
- [ ] Domain models are in `Balenthiran.LanguageVocab.DomainModels`, data models in `Balenthiran.LanguageVocab.DataModels`
