...
```
Balenthiran.LanguageVocab/
├── Balenthiran.LanguageVocab.WebApi         # Entry point: routes, DI, OpenAPI, middleware
...
├── Balenthiran.LanguageVocab.Services       # Business logic implementations
│   └── Mapper.cs               # EntityModel ↔ DomainModel mappings
├── Balenthiran.LanguageVocab.Abstractions   # Interfaces only — no implementations
...
├── Balenthiran.LanguageVocab.EntityModels   # Database entity classes (anemic POCOs)
├── Balenthiran.LanguageVocab.DomainModels   # Rich business-layer objects
└── Balenthiran.LanguageVocab.DataModels     # Request/Response DTOs (public API contract)
```
