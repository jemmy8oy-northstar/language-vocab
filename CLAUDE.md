# CLAUDE.md — language-vocab

Mandarin vocabulary trainer (see `docs/DESIGN.md`). Monorepo scaffolded from [web-template](https://github.com/jemmy8oy-northstar/web-template): .NET 10 backend + React 19 frontend.
...
│   ├── Balenthiran.LanguageVocab.WebApi/       # Entry point: routes, DI, OpenAPI
│   ├── Balenthiran.LanguageVocab.Services/     # Business logic, AutoMapper profiles
│   ├── Balenthiran.LanguageVocab.Abstractions/ # Service interfaces + model interfaces
│   ├── Balenthiran.LanguageVocab.Database/     # EF Core DbContext + migrations
│   ├── Balenthiran.LanguageVocab.EntityModels/ # Database entities (anemic POCOs)
│   ├── Balenthiran.LanguageVocab.DomainModels/ # Rich business-layer objects
│   └── Balenthiran.LanguageVocab.DataModels/   # Request/Response DTOs
...
```bash
# Backend (Terminal 1)
cd backend && dotnet run --project Balenthiran.LanguageVocab.WebApi
...
cd backend && dotnet ef database update \
  --project Balenthiran.LanguageVocab.Database \
  --startup-project Balenthiran.LanguageVocab.WebApi
```
...
- [ ] No concrete types defined in `Balenthiran.LanguageVocab.Abstractions`
- [ ] Service interface methods return **interfaces**, not concrete types (`IRatio`, not `Ratio`)
- [ ] `Balenthiran.LanguageVocab.Abstractions.csproj` has **zero** `<ProjectReference>` entries to concrete projects
...
- [ ] Domain models in `Balenthiran.LanguageVocab.DomainModels`, data models in `Balenthiran.LanguageVocab.DataModels`
